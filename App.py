...
class PopUp(object):
    def __init__(self,master):
        ...
        self.l = Label(top, text="Enter Task", padx=60, pady=20,fg = "#074a35", bg="#79e8c6",font=("Comic Sans MS",15)).grid(row=0,column=0,columnspan=6)
        self.l1 = Label(top, text="Task Name", padx=40, pady=20, font=(None, 10)).grid(row=1, column=0)
        self.l1 = Label(top, text="Duration", font=(None, 10)).grid(row=1, column=3,columnspan=2)
        self.entry_name = Entry(top)
        self.entry_name.grid(row=3,column=0,columnspan=3)

        self.entry_hour = Entry(top,width = 2)
        self.entry_hour.insert(END, "00")
        self.entry_min = Entry(top,width = 2)
        self.entry_min.insert(END, "00")
        self.entry_sec = Entry(top,width = 2)
        self.entry_sec.insert(END, "00")

        self.photo_down = PhotoImage(file="down.png")
        self.photo_up = PhotoImage(file = "up.png")
        self.up1 = Button(top, image = self.photo_up,height = 10, width = 10,command=lambda: self.increase("hour")).grid(row=2, column=3)
        self.up2 = Button(top, image= self.photo_up,height = 10, width = 10,command=lambda: self.increase("min")).grid(row=2, column=4)
        self.up3 = Button(top, image= self.photo_up,height = 10, width = 10,command=lambda: self.increase("sec")).grid(row=2, column=5)
        ...
        self.down1 = Button(top, image=self.photo_down,height = 10, width = 10,command=lambda: self.decrease("hour")).grid(row=4, column=3)
        self.down2 = Button(top, image=self.photo_down,height = 10, width = 10,command=lambda: self.decrease("min")).grid(row=4, column=4)
        self.down3 = Button(top, image=self.photo_down,height = 10, width = 10,command=lambda: self.decrease("sec")).grid(row=4, column=5)

        self.b = Button(top, text='Save', padx=20, pady=5,command=self.cleanup).grid(row=5,column=0,columnspan=6)
...
class CountDown:
    ...
    def add_task(self,task):
        ...
        task_name = Label(root, text=task.name)
        task_name.grid(row=len(self.task_list), column=0)
        task_name.bind("<Button-1>", lambda e: self.clock_trigger(self.task_list.index(task)))
        duration = Label(root, text=task.duration[0]+" : "+task.duration[1]+" : "+task.duration[2])
        duration.grid(row=len(self.task_list), column=1)
    ...
    def decrement(self,index):
        ...
        duration = Label(root, text=str(new_duration[0])+" : "+str(new_duration[1])+" : "+str(new_duration[2]))
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Effective Countdown App")
    m=main(root)
    root.mainloop()
